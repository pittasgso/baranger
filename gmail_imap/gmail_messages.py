...
class gmail_messages:
    ...
    def process(self,mailbox,limit=None):
	...
	if (limit != None):
	        fetch_list = string.split(data[0])[-limit:]# limit to N most recent messages in mailbox, this is where pagination should be implemented
	else:
	        fetch_list = string.split(data[0])		#do not limit ourselves to how many messages we download
    ...
    def getMessage(self, uid, forceTextOnly=True):
        if(not self.server.loggedIn):
            self.server.login()
